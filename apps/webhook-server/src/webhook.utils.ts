...
const verifySignature = (req: Request) => {
  ...
  const untrusted = typeof headerValue === 'string' ? Buffer.from(headerValue, 'utf-8') : Buffer.alloc(0)
...
const executeDockerCompose = (project: string, successCallBack: () => void, errorCallBack: () => void) => {
