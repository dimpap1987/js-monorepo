...
export function withLocale(
  ...
  return async function middlewareLocale(request: NextRequest): Promise<NextResponse<unknown>> {
    ...
    if (response.headers.get('location')) {
      applyLocaleToResponse(response, localeResult, localizationConfig)
      return response
    }

    // For non-redirect responses, create new response with locale header
    // This is required because request.headers is read-only
    ...
    applyLocaleToResponse(localeResponse, localeResult, localizationConfig)
