...
export function isValidType(value: any): value is RegisterDialogErrorComponentType['type'] {
  return initialRegisterValidations.some((validation) => validation.type === value || value === 'custom')
}

export const handleValidationErrros = (
  errors: FieldErrors<any> | string[],
  callback: React.Dispatch<React.SetStateAction<RegisterDialogErrorComponentType[]>>
) => {
  const formErrors = Object.values(errors ?? {})
  const validationErrors: RegisterDialogErrorComponentType[] = formErrors.map((error: any) => ({
    status: 'invalid',
  ...
  callback((prev) => {
    ...
    const filteredValidations = prev.filter(
      (validation) => !validationErrors?.some((newValidation) => newValidation.type === validation.type)
    )

    // Map over filtered validations, update status to 'valid' for invalid ones
    const updatedValidations: RegisterDialogErrorComponentType[] = filteredValidations.map((validation) =>
      validation.status === 'invalid' ? { ...validation, status: 'valid' } : validation
    )
    ...
    const orderedVal = [...updatedValidations, ...validationErrors].sort(
      (a, b) => validationOrderTypes.indexOf(a.type) - validationOrderTypes.indexOf(b.type)
    )
    return orderedVal
  })
}
