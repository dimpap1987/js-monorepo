...
import { PlanCardActionsType, PlanCardContainerType, PlanCardContentType, PlanCardProps, Subscription } from '../types'

function getSubscriptionMessage({ subscription, name }: { subscription: Subscription; name: string }) {
  ...
  switch (subscription.status) {
    case 'active':
      return subscription.cancelAt ? (
        <li>
          Your <strong className="capitalize">{name}</strong> will be canceled at the end of the period{' '}
          <strong>{moment(subscription.cancelAt).format('YYYY-MM-DD')}</strong> at{' '}
          <strong>{moment(subscription.cancelAt).format('hh:mm A')}</strong>.
          <br />
        </li>
      ) : (
        <li>
          Your <strong className="capitalize">{name}</strong> will renew automatically on{' '}
          <strong>{moment(subscription.currentPeriodEnd).format('YYYY-MM-DD')}</strong> at{' '}
          <strong>{moment(subscription.currentPeriodEnd).format('hh:mm A')}</strong>
          .
          <br />
    ...
    case 'trialing':
      return (
        <li>
          You're currently on a <strong>trial</strong> for the <strong className="capitalize">{name}</strong>. Your
          trial will end on <strong>{moment(subscription.trialEnd).format('YYYY-MM-DD')}</strong> at{' '}
          <strong>{moment(subscription.trialEnd).format('hh:mm A')}</strong>. <br />
          To continue enjoying the benefits, ensure you subscribe before the trial ends.
        </li>
      )
    case 'canceled':
      return (
        <li>
          Your <strong className="capitalize">{name}</strong> plan has been cancelled. Date of Cancellation:{' '}
          <strong>{moment(subscription.cancelAt).format('YYYY-MM-DD')}</strong> at{' '}
          <strong>{moment(subscription.cancelAt).format('hh:mm A')}</strong>,
        </li>
      )
...
export function ActiveSubscriptionLabel({ subscribed }: { subscribed: boolean }) {
  return (
    subscribed && (
      <div
...
const renderSubscribedActions = (
  ...
  switch (sub.status) {
    case 'active':
      return sub.cancelAt ? (
        <DpButton size="large" className="w-full" loading={isLoading} onClick={onCheckout}>
        ...
        <div>
          <div className="h-10 rounded-md px-8 bg-background border border-border text-foreground flex items-center justify-center font-semibold shadow-md relative">
            <span className="text-base whitespace-break-spaces">Current Plan</span>
            <CancelActionButton
              ...
              smallMessage={
                <div className="text-xs">
                  ...
                  <strong>{moment(sub.currentPeriodEnd).format('YYYY-MM-DD')}</strong> at{' '}
                  <strong>{moment(sub.currentPeriodEnd).format('hh:mm A')}</strong>. You will retain access to all
                  features until this time, even after cancellation.
                </div>
              }
            />
          </div>
    ...
    default:
      return <CancelActionButton isDialogOpen={isDialogOpen} onCancel={onCancel} setDialogOpen={setDialogOpen} />
  }
}
...
export function PlanCardActions({
  ...
  if (!isLoggedIn) {
    return (
      <DpButton className="w-full" size="large" loading={isLoading} onClick={onCheckout}>
  ...
  return isSubscribed && subscription ? (
    renderSubscribedActions(subscription, isLoading, isDialogOpen, setDialogOpen, onCheckout, onCancel)
  ) : (
    <DpButton
      size="large"
      className="w-full"
