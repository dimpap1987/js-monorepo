...
const NotificationItem = React.memo(({ content, onRead }: NotificationItemProps) => {
  return (
    ...
    >
      <GoDotFill className={`text-2xl mr-2 shrink-0 ${content.isRead ? 'text-gray-500' : 'text-white'}`} />
      <div
      ...
      ></div>
      <DropdownMenuShortcut>{humanatizeNotificationDate(content.notification.createdAt)} ago</DropdownMenuShortcut>
    </DropdownMenuItem>
  )
})
