...
export class GithubOauthStrategy extends PassportStrategy(Strategy, 'github') {
  ...
  async validate(accessToken: string, _refreshToken: string, profile: any, done: oauth2.VerifyCallback) {
    const userEmails = await this.getUsersEmailsResponse(accessToken)
    const emailObj = userEmails?.data?.find((d: { primary: any; verified: any }) => d.primary && d.verified)
